...
class CallkitIncomingActivity : Activity() {
    ...
    inner class EndedCallkitIncomingBroadcastReceiver : BroadcastReceiver() {
        override fun onReceive(context: Context?, intent: Intent?) {
            Log.e("TestReceiver", "onReceive: test")
            if (!isFinishing) {
    ...
    lateinit private var endedCallkitIncomingBroadcastReceiver : BroadcastReceiver
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        endedCallkitIncomingBroadcastReceiver = object : BroadcastReceiver() {
            override fun onReceive(context: Context, intent: Intent) {
                Log.e("TestReceiver", "onReceive: test2")
                if (!isFinishing) {
                    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
                        finishAndRemoveTask()
                    } else {
...
//        )
    ...
    override fun onResume() {
        Log.e("TestReceiver", "onReceive: test1")
        super.onResume()
        registerReceiver(endedCallkitIncomingBroadcastReceiver, IntentFilter(
            CallkitIncomingBroadcastReceiver.ACTION_CALL_DECLINE
        ));
    }
    ...
    public fun finishTimeout(data: Bundle?, duration: Long) {
