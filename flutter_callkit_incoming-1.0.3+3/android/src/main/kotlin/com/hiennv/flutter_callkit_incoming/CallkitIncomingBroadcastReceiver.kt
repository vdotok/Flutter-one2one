...
class CallkitIncomingBroadcastReceiver : BroadcastReceiver() {
    ...
    companion object {
        ...
        fun getIntentDeclineTask(context: Context, data: Bundle?) =
            Intent(context, CallkitIncomingBroadcastReceiver::class.java).apply {
                action = "${context.packageName}.${ACTION_CALL_DECLINE}"
                putExtra(EXTRA_CALLKIT_INCOMING_DATA, data)
            }
    ...
    override fun onReceive(context: Context, intent: Intent) {
        ...
        when (action) {
            ...
            "${context.packageName}.${ACTION_CALL_DECLINE}" -> {
                android.util.Log.e("TestReceiver", "onMethodCall: endAllCalls2", )
                try {
                    context.sendBroadcast(Intent(ACTION_CALL_DECLINE))
                    callkitNotificationManager.clearIncomingTask(data)
            ...
            "${context.packageName}.${ACTION_CALL_TIMEOUT}" -> {
                try {
                    context.sendBroadcast(Intent(CallkitIncomingActivity.ACTION_ENDED_CALL_INCOMING))
                    callkitNotificationManager.clearIncomingTask(data)
    ...
    private fun sendEventFlutter(event: String, data: Bundle) {
        ...
        val forwardData = mapOf(
                ...
                "nameCaller" to data.getString(EXTRA_CALLKIT_NAME_CALLER, "woooo"),
